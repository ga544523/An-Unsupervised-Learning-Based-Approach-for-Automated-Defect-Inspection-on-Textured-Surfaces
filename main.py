...
encoder_decoder512 = encoder_decoder512()
encoder_net = encoder_decoder512.to(device1)

encoder_decoder256 = encoder_decoder256()
encoder_net = encoder_decoder256.to(device1)

encoder_decoder128 = encoder_decoder128()
encoder_net = encoder_decoder128.to(device1)
...
encoder_decoder512_optimizer = torch.optim.Adam(encoder_decoder512.parameters(), lr=0.0001)
encoder_decoder256_optimizer = torch.optim.Adam(encoder_decoder256.parameters(), lr=0.0001)
encoder_decoder128_optimizer = torch.optim.Adam(encoder_decoder128.parameters(), lr=0.0001)
...
def train_decoder(traindata_non_defect,traindata_original,currentstep):
    ...
    for b in range(0,batchsize,1):
        ...
        ii=random.randint(patchsize+1, size-patchsize-1)
        jj=random.randint(patchsize+1, size-patchsize-1)
    ...
    if (currentstep == 0):
        output = encoder_decoder512(total_patchimg.to(device1))
        output = output.to(device1)
    if (currentstep == 1):
        output = encoder_decoder256(total_patchimg.to(device1))
        output = output.to(device1)
    if (currentstep == 2):
        output = encoder_decoder128(total_patchimg.to(device1))
        output = output.to(device1)

    total_compareimg=total_compareimg.to(device1)

    if(count_update1%print_image_interval==0):
        torchvision.utils.save_image(output, '/home/ipx/result/test'+'type1  '+ str(count_update1) + '.png')
    elif (count_update2%print_image_interval==0):
        torchvision.utils.save_image(output, '/home/ipx/result/test' + 'type2  ' + str(count_update2) + '.png')
    elif (count_update3 % print_image_interval == 0):
        torchvision.utils.save_image(output, '/home/ipx/result/test' + 'type3  ' + str(count_update3) + '.png')
...
for p in encoder_decoder512.parameters():
    # print(p.size())
...
for t in range(0, 1000000, 1):
    ...
    for j in range(0, len(positive) , 1):
        ...
        for step in range(0,3,1):
            ...
            if(step==0):
                torch.nn.utils.clip_grad_norm_(encoder_decoder512.parameters(), max_norm=3)
                encoder_decoder512_optimizer.step()
            if(step==1):
                torch.nn.utils.clip_grad_norm_(encoder_decoder256.parameters(), max_norm=3)
                encoder_decoder256_optimizer.step()
            if(step==2):
                torch.nn.utils.clip_grad_norm_(encoder_decoder128.parameters(), max_norm=3)
        ...
        traindata_original = 0
    if(t%100==99):
        torch.save(encoder_decoder512, 'encoder_decoder512.pkl')
        torch.save(encoder_decoder256, 'encoder_decoder256.pkl')
        torch.save(encoder_decoder128, 'encoder_decoder128.pkl')
